       ...
       // }
	    ...
	    stage ('Login to ECR') {
		    ...
		    withEnv(["AWS_ACCESS_KEY_ID=${env.AWS_ACCESS_KEY_ID}", "AWS_SECRET_ACCESS_KEY=${env.AWS_SECRET_ACCESS_KEY}", "AWS_DEFAULT_REGION=${env.AWS_DEFAULT_REGION}"]) {
                    sh 'docker login -u AWS -p $(aws ecr get-login-password --region us-east-1) 612277859916.dkr.ecr.us-east-1.amazonaws.com' //985729960198.dkr.ecr.eu-west-2.amazonaws.com'
		    }
